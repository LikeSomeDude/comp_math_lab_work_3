...
bool should_recalculate_jacobian(std::size_t iteration, const NewtonOptions& options) {
    if (iteration == 0) {
        return true;
    }
    ...
    if (options.jacobian_recalculation_period == 0
        || options.jacobian_recalculation_period == std::numeric_limits<std::size_t>::max()) {
        return false;
    }

    return (iteration - options.full_jacobian_iterations + 1) % options.jacobian_recalculation_period == 0;
}
}
...
NewtonResult solve_newton(const Vector& x0, const NewtonOptions& options) {
    ...
    LuFactorization current_lu;

    for (std::size_t iteration = 0; iteration < options.max_iterations; ++iteration) {
        const Vector f = system_values(result.x);
        result.residual_norm = norm_inf(f);

        if (result.residual_norm < options.eps) {
        ...
        if (iteration == 0 || should_recalculate_jacobian(iteration, options)) {
            ...
            current_lu = decompose_lu(jacobian(result.x));
            ++result.jacobian_recalculations;
            result.linear_solve_stats.additions += current_lu.stats.additions;
            result.linear_solve_stats.multiplications += current_lu.stats.multiplications;
        ...
        if (norm_inf(step.x) < options.eps) {
            result.residual_norm = norm_inf(system_values(result.x));
