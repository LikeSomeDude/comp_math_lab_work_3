...
Vector system_values(const Vector& x) {
    ...
    Vector f(10);
    f[0] = std::cos(x2 * x1) - std::exp(-(3.0 * x3)) + x4 * x5 * x5 - x6 - std::sinh(2.0 * x8) * x9 + 2.0 * x10 + 2.000433974165385440;
    f[1] = std::sin(x2 * x1) + x3 * x9 * x7 - std::exp(-x10 + x6) + 3.0 * x5 * x5 - x6 * (x8 + 1.0) + 10.886272036407019994;
    f[2] = x1 - x2 + x3 - x4 + x5 - x6 + x7 - x8 + x9 - x10 - 3.1361904761904761904;
    f[3] = 2.0 * std::cos(-x9 + x4) + x5 / (x3 + x1) - std::sin(x2 * x2) + std::pow(std::cos(x7 * x10), 2.0) - x8 - 0.1707472705022304757;
    f[4] = std::sin(x5) + 2.0 * x8 * (x3 + x1) - std::exp(-x7 * (-x10 + x6)) + 2.0 * std::cos(x2) - 1.0 / (-x9 + x4) - 0.3685896273101277862;
    f[5] = std::exp(x1 - x4 - x9) + x5 * x5 / x8 + std::cos(3.0 * x10 * x2) / 2.0 - x6 * x3 + 2.0491086016771875115;
    f[6] = std::pow(x2, 3.0) * x7 - std::sin(x10 / x5 + x8) + (x1 - x6) * std::cos(x4) + x3 - 0.7380430076202798014;
    f[7] = x5 * std::pow(x1 - 2.0 * x6, 2.0) - 2.0 * std::sin(-x9 + x3) + 1.5 * x4 - std::exp(x2 * x7 + x10) + 3.5668321989693809040;
    f[8] = 7.0 / x6 + std::exp(x5 + x4) - 2.0 * x2 * x8 * x10 * x7 + 3.0 * x9 - 3.0 * x1 - 8.4394734508383257499;
    f[9] = x10 * x1 + x9 * x2 - x8 * x3 + std::sin(x4 + x5 + x6) * x7 - 0.78238095238095238096;
