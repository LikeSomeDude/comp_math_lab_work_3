...
namespace {
// проверяет что матрица не пустая и квадратная
...
// проверяет что размеры L, U и перестановки совпадают
bool is_square_size_compatible(const LuFactorization &lu) {
  return lu.l.size() == lu.u.size() && lu.l.size() == lu.permutation.size();
}
} // namespace

// возвращает общее количество арифметических операций
long long OperationStats::total() const {
  return additions + multiplications + divisions;
}

// обнуляет счетчики операций
...
LuFactorization decompose_lu(Matrix a) {
  ...
  for (std::size_t k = 0; k < n; ++k) {
    // частичный выбор главного элемента в текущем столбце
    std::size_t pivot = k;
    ...
    if (std::abs(result.u[pivot][k]) < 1e-10) { // проверка на вырожденность
...
LinearSolveResult solve_with_lu(const LuFactorization &lu, const Vector &b) {
  ...
  const std::size_t n = lu.l.size(); // размер системы
...
// строит LU-разложение и сразу решает СЛАУ
